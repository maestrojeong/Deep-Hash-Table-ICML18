...
# ../../utils 
from evaluation import evaluate_metric_te_tr, evaluate_hash_te_tr
...
# ../../tfops
from transform_op import apply_tf_op, get_recall_at_1_efficient
...
class DeepMetric:
    ...
    def train(self, epoch, save_dir, board_dir):
        ...
        for epoch_ in range(self.start_epoch, self.start_epoch+epoch):
            ...
            else: self.val_embed = custom_apply_tf_op(inputs=self.val_image, output_gate=self.embed) # triplet
            ...
            self.logger.info("Epoch({}/{}) train loss = {} val p@1 = {}".format(epoch_ + 1, epoch+self.start_epoch, train_epoch_loss, val_p1))
    ...
    def test_th(self, activate_k, k_set):
        ...
        train_k_activate = activate_k_2D(self.train_embed, k=activate_k, session=self.sess) # [ntrain, args.m]
        self.regen_session()
    ...
    def test_vq(self, activate_k, k_set):
        ...
        te_te_query_value = self.test_kmc.k_hash(self.test_embed, self.sess) # [ntest, args.m] center test
        te_tr_query_value = self.train_kmc.k_hash(self.test_embed, self.sess) # [ntest, args.m] center train
        self.regen_session()

        te_te_query_key = activate_k_2D(te_te_query_value, k=activate_k, session=self.sess) # [ntest, args.m]
        test_hash_key = te_te_query_key

        te_tr_query_key = activate_k_2D(te_tr_query_value, k=activate_k, session=self.sess) # [ntest, args.m]
        train_hash_key = activate_k_2D(self.train_kmc.k_hash(self.train_embed, self.sess), k=activate_k, session=self.sess) # [ntrain, args.m]
        self.regen_session()
